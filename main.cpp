...
int main(){
    ...
    for(int i = 0; i < N_test; ++i){
        ...
        mfield_exact_convtest.initHess(gaussian_ic);

        // Hessian Operator
        GeneralizedHessOp<double,DiffType::Forward,DiffType::Forward,DiffType::Forward> hessOp_convtest(field_convtest);

        N = field_convtest.getN();
        N_ext = field_convtest.getN_ext();
        mesh_sizes[i] = N[0];

        rel_error[i] = hessError(mfield_exact_convtest, hessOp_convtest);
